...
from rango.models import User, Movie, Category, Movie_rating
...
def get_movie_info():
    ...
    with open(path) as fb:
        ...
        # 读取title信息 id,name,url,time,category,release_time,intro,directores,writers,starts
        ...
        for i, line in enumerate(reader):
            ...
            m.save()
            # 建立类型关系
            for category in line[title_dct['category']].split('|'):
                # 找到类型 category_object
                go = Category.objects.filter(name=category).first()
                # print(go)
                m.category.add(go)
            if i % 1000 == 0:
                print(i)    # 控制台查看进度用
...
def get_user_and_rating():
    ...
    for user_id, ratings in user_id_dct.items():
        u = User.objects.create(
            name=user_id, password=user_id, email=f'{user_id}@1.com')
        # 必须先保存
        u.save()
        # 开始加入评分记录
        for imdb_id, rating in ratings.items():
            ...
            relation = Movie_rating(
                user=u, movie=movie, score=rating, comment='')
            relation.save()
            # break
